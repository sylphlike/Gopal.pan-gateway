...
/**
 * 包装业务系统返回
 * <p> 处理请求业务系统返回404，返回结果为空串 的情况做结果处理，返回统一
 *     的数据格式,springGateway 对返回数据流使用了分段传输，当数据大小过大时 会出现返回数据不完整情况,使用fluxboy.buffer(),方
 *     式获取缓冲中所有的数据后再处理业务逻辑
 * <p>  time 30/09/2020 09:33  星期三 【dd/MM/YYYY HH:mm】 </p>
